// Service Worker für Admiral Bundesliga Sixpack PWA
const CACHE_NAME = 'bundesliga-sixpack-v1.0.0';
const STATIC_CACHE = 'bundesliga-sixpack-static-v1.0.0';
const DYNAMIC_CACHE = 'bundesliga-sixpack-dynamic-v1.0.0';
...
self.addEventListener('fetch', (event) => {
  ...
  // Cache Strategy: Network First für API calls und HTML
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: event.data ? event.data.text() : 'Neue Bundesliga Sixpack Benachrichtigung',
  ...
  event.waitUntil(
    self.registration.showNotification('Bundesliga Sixpack', options)
  );
});
